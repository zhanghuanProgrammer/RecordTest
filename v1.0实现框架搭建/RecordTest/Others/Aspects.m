...
//struct类型名
static NSString *qhd_structName(const char *argumentType) {
...
static BOOL isCGRect           (const char *type) {return [qhd_structName(type) isEqualToString:@"CGRect"];}
static BOOL isCGPoint          (const char *type) {return [qhd_structName(type) isEqualToString:@"CGPoint"];}
static BOOL isCGSize           (const char *type) {return [qhd_structName(type) isEqualToString:@"CGSize"];}
static BOOL isCGVector         (const char *type) {return [qhd_structName(type) isEqualToString:@"CGVector"];}
static BOOL isUIOffset         (const char *type) {return [qhd_structName(type) isEqualToString:@"UIOffset"];}
static BOOL isUIEdgeInsets     (const char *type) {return [qhd_structName(type) isEqualToString:@"UIEdgeInsets"];}
static BOOL isCGAffineTransform(const char *type) {return [qhd_structName(type) isEqualToString:@"CGAffineTransform"];}

//是否struct类型
static BOOL qhd_isStructType(const char *argumentType) {
    NSString *typeString = [NSString stringWithUTF8String:argumentType];
    return ([typeString hasPrefix:@"{"] && [typeString hasSuffix:@"}"]);
}

//获取方法参数
static NSArray *qhd_method_arguments(NSInvocation *invocation) {
...
//获取方法返回值
static id getReturnValue(NSInvocation *invocation){
...
#pragma mark - AspectBlock

@interface AspectBlock : NSObject

@property (strong, nonatomic) NSString *targetClassName;
@property (copy, nonatomic) AspectBeforeBlock before;
@property (copy, nonatomic) AspectAfterBlock  after;

@end

@implementation AspectBlock
...
#pragma mark - AspectBlockInstance
...
@implementation AspectBlockInstance

+ (instancetype)sharedAspectBlockInstance {
    static AspectBlockInstance *_sharedAspectBlockInstance = nil;
    ...
    if (objectContainer.insteadAspects.count || classContainer.insteadAspects.count) {
        ...
        do {
            if ((respondsToAlias = [klass instancesRespondToSelector:aliasSelector])) {
                ...
                [block rundAfter:invocation.target sel:originalSelector args:argList interval:interval deep:deep retValue:getReturnValue(invocation)];
                deep--;
                break;
            }
