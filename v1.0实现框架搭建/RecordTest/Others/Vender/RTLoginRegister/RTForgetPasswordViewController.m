...
- (void)getVerficationCodeAction{
    ...
    //    [SDNetworking postWithUrl:[NSString stringWithFormat:@"http://dk.juyoux.cn/DKapi/SendVerification"] parameters:@{@"UserPhone":phone.text,@"Types":@"1",@"v":md5} shouldShowHud:YES forVC:self success:^(id json) {
    //
    ...
    //        [MBProgressHUD showMessage:@"获取验证码失败,请重试!"];
    //        getVerficationCodeButton.enabled=YES;
    //    }reloadBlock:^{
    //
    //    }];
...
- (void)changePswWith:(NSString*)userName password:(NSString*)password verCode:(NSString*)verCode{
    //    NSString *urlMd5=[NSString stringWithFormat:@"http://dk.juyoux.cn/DKapi/PwRetrieval?UserPhone=%@&Pw=%@&code=%@&device=%@&version=%@&timestamp=%@",userName,password,verCode,[SDTool deviceID],[SDTool version],[NSString stringWithFormat:@"%lld",[DateTools getCurInterval]]];
    //    NSString *md5=[ZHNSString md5:urlMd5];
    //    NSMutableDictionary *parameters=[NSMutableDictionary dictionaryWithDictionary:@{@"UserPhone":userName,@"Pw":password,@"code":verCode,@"v":md5}];
    //    [SDNetworking postWithUrl:[NSString stringWithFormat:@"http://dk.juyoux.cn/DKapi/PwRetrieval"] parameters:parameters shouldShowHud:YES forVC:self success:^(id json) {
    //
    //        NSDictionary  *dic=(NSDictionary *)json;
    //        NSLog(@"修改密码:\n%@",dic);
    //        if ([dic isKindOfClass:[NSDictionary class]]) {
    ...
    //        }
    //    } failure:^(NSError *error) {
    //        [MBProgressHUD showMessage:@"网络错误,请重试!"];
    //    }reloadBlock:^{
...
- (void)createTextFields{
    ...
    UIButton* landBtn = [self createButtonFrame:CGRectMake(10, bgView.frame.size.height + bgView.frame.origin.y + 30, self.view.frame.size.width - 20, 37) backImageName:nil title:@"确认修改" titleColor:[UIColor whiteColor] font:[UIFont systemFontOfSize:17] target:self action:@selector(changePsw)];
