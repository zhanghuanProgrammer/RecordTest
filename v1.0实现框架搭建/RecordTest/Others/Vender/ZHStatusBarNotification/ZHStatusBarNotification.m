...
+ (ZHStatusBarStyle*)defaultStyleWithName:(NSString*)styleName;
@end
...
@interface ZHStatusBarNotification () <CAAnimationDelegate>
@property (nonatomic, strong, readonly) UIWindow *overlayWindow;
@property (nonatomic, strong, readonly) UIView *progressView;
@property (nonatomic, strong, readonly) ZHStatusBarView *topBar;

@property (nonatomic, strong) NSTimer *dismissTimer;
@property (nonatomic, assign) CGFloat progress;

@property (nonatomic, weak) ZHStatusBarStyle *activeStyle;
@property (nonatomic, strong) ZHStatusBarStyle *defaultStyle;
@property (nonatomic, strong) NSMutableDictionary *userStyles;
@end

@implementation ZHStatusBarNotification
...
+ (ZHStatusBarNotification*)sharedInstance {
  static dispatch_once_t once;
  static ZHStatusBarNotification *sharedInstance;
  dispatch_once(&once, ^ {
...
+ (void)setDefaultStyle:(JDPrepareStyleBlock)prepareBlock;
{
  ...
  ZHStatusBarStyle *style = [[self sharedInstance].defaultStyle copy];
...
{
  self.defaultStyle = [ZHStatusBarStyle defaultStyleWithName:JDStatusBarStyleDefault];

  self.userStyles = [NSMutableDictionary dictionary];
  for (NSString *styleName in [ZHStatusBarStyle allDefaultStyleIdentifier]) {
    [self.userStyles setObject:[ZHStatusBarStyle defaultStyleWithName:styleName] forKey:styleName];
  }
}
                   ...
                   prepare:(JDPrepareStyleBlock)prepareBlock;
{
  ...
  ZHStatusBarStyle *style = [self.defaultStyle copy];
...
- (ZHStatusBarView*)topBar;
{
  if(_topBar == nil) {
    _topBar = [[ZHStatusBarView alloc] init];
    [self.overlayWindow.rootViewController.view addSubview:_topBar];

    ZHStatusBarStyle *style = self.activeStyle ?: self.defaultStyle;
